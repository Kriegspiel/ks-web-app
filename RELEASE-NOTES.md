...
- **Bot Catalog**: move Qwen Plus from T3 Strong to T2 Club across the
  subscription matrix, lobby bot picker, and bot profile tier cards.

## ks-web-app frontend v. 1.3.183

- **Subscription**: make the subscription page public for logged-out visitors,
  showing a free-profile invitation while keeping current-level and billing
  controls limited to signed-in users.

## ks-web-app frontend v. 1.3.182

- **Subscription**: hide Nemotron Nano from the visible tier matrix while
  preserving direct bot profile access.

## ks-web-app frontend v. 1.3.181

- **Lobby**: center fixed toast messages so copy confirmations read cleanly at
...
- **Profile**: show Stockfish Wild 16 as a T1 bot and add a TD badge for
  deactivated bot accounts on profile pages.
...
- **Subscription**: add a bot availability disclaimer with contact links for
  provider outages, API downtime, or inconsistent bot behavior.
...
- **Lobby**: make open-game codes copyable with explicit feedback and split
  open-game metadata into clearer rules, color, and time/code lines.

## ks-web-app frontend v. 1.3.168

- **Subscription**: make the guest-account subscription blocker a prominent
  yellow warning so users notice they must convert before subscribing.
...
- **Bot Matrix Report**: add a review outcome selector that carries
  end-condition filters, including no resignations/timeouts, into matrix game
  history links.
...
- **Bot Matrix Report**: add row-bot and column-bot selection menus for the
  outcome matrix, with all bots selected by default.
...
## ks-web-app frontend v. 1.3.109

- **Bot Matrix Report**: default the time period to Week and show the selected
  report window beside the period dropdown.

## ks-web-app frontend v. 1.3.108

- **Bot Matrix Report**: show available opponent-side token and spend averages
  in matrix cells when the row bot has no recorded usage for that matchup.
...
- **Bot Matrix Report**: freeze the Bot totals table header and player column,
  and align Bot totals sorting/filter controls with the game-history table.
