...
export async function getMyActiveGames() { try { const response = await api.get('/api/game/mine/active'); return response.data } catch (error) { throw normalizeError(error, 'Unable to load your games right now.') } }
...
export async function getGameT3Review(gameId) { try { const response = await api.get(`/api/game/${encodeURIComponent(gameId)}/review/t3`); return response.data } catch (error) { throw normalizeError(error, 'Unable to load T3 review right now.') } }
export async function getGamePublicStatus(gameId) { try { const response = await api.get(`/api/game/${encodeURIComponent(gameId)}/public-status`); return response.data } catch (error) { throw normalizeError(error, 'Unable to load game status right now.') } }
...
export const techApi = {
  ...
  async getBotMatrixReport(period = "lifetime", outcomes = []) {
    ...
    if (Array.isArray(outcomes) && outcomes.length > 0) {
      params.outcomes = outcomes.join(",")
    }
    const response = await api.get("/api/tech/bot-matrix-report", { params })
    return response.data
  },
