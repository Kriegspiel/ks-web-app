...
export default function LobbyPage() {
  ...
  return (<main className="page-shell lobby-page"><h1>Lobby</h1><p>Signed in as {signedInAs}.</p>{actionError ? <p className="auth-error" role="alert">{actionError}</p> : null}
    <section className="lobby-card" aria-labelledby="create-game-heading"><h2 id="create-game-heading">Create game</h2><form onSubmit={handleCreateGame} className="lobby-create-form"><fieldset className="lobby-opponent-picker"><legend>Who is this game against?</legend><div className="lobby-opponent-options"><label><input type="radio" name="opponent-type" checked={opponentType === 'human'} onChange={() => setOpponentType('human')} /> Human</label><label><input type="radio" name="opponent-type" checked={opponentType === 'bot'} onChange={() => setOpponentType('bot')} /> Bot</label></div></fieldset>{opponentType === 'bot' ? <div className="lobby-bot-picker"><label htmlFor="bot-picker">Bot opponent</label><select id="bot-picker" value={selectedBotId} onChange={(event) => setSelectedBotId(event.target.value)}><option value="">Select a bot</option>{bots.map((bot) => <option key={bot.bot_id} value={bot.bot_id}>{bot.display_name}</option>)}</select>{selectedBotId ? <p className="lobby-meta">{bots.find((bot) => bot.bot_id === selectedBotId)?.description}</p> : null}{botsError ? <p className="auth-error" role="alert">{botsError}</p> : null}</div> : <p className="lobby-meta">Create a waiting game and share the join code with another human.</p>}<button type="submit" disabled={creatingGame}>{creatingGame ? 'Creating…' : opponentType === 'bot' ? 'Create bot game' : 'Create waiting game'}</button></form>{createError ? <p className="auth-error" role="alert">{createError}</p> : null}{createResult && createResult.state === 'waiting' ? <div className="lobby-created-game" role="status" aria-live="polite"><p><strong>Join code:</strong> <code>{createResult.game_code}</code></p><p><strong>Share link:</strong> <a href={shareJoinUrl}>{shareJoinUrl}</a></p><p><strong>State:</strong> {waitingGameId ? 'Waiting for opponent…' : createResult.state}</p></div> : null}</section>
