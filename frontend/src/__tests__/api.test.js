...
describe("user helpers", () => {
  ...
  it("tech_reports_use_expected_endpoints", async () => { const getSpy = vi.spyOn(api, "get").mockResolvedValue({ data: {} }); await techApi.getBotsReport(10); await techApi.getGuestsReport(); await techApi.getUsersReport(); await techApi.getAcquisitionReport(7); expect(getSpy).toHaveBeenNthCalledWith(1, "/api/tech/bots-report", { params: { days: 10 } }); expect(getSpy).toHaveBeenNthCalledWith(2, "/api/tech/guests-report"); expect(getSpy).toHaveBeenNthCalledWith(3, "/api/tech/users-report"); expect(getSpy).toHaveBeenNthCalledWith(4, "/api/tech/acquisition-report", { params: { days: 7 } }) })
