...
describe("LobbyPage", () => {
  ...
  it("shows_bot_picker_and_creates_bot_game", async () => {
    ...
    expect(screen.getByRole("option", { name: "1342 - LLM GPT-4.5 Nano (bot) (128 ply limit)" })).toBeInTheDocument()
    ...
    expect(botOptions).toEqual([
      "Select a bot",
      "1342 - LLM GPT-4.5 Nano (bot) (128 ply limit)",
      "1201 - Random Bot",
      "1200 - Random Any Bot",
    ])
  ...
  it("updates_the_bot_description_when_selecting_gpt_nano", async () => {
    ...
    expect(screen.getByText("LLM GPT-4.5 Nano (bot) Kriegspiel model bot.")).toBeInTheDocument()
  })
  ...
  it("does_not_treat_missing_bot_rulesets_as_support_for_new_variants", async () => {
    ...
    expect(screen.getByRole("option", { name: "1229 - LLM GPT-4.5 Nano (bot)" })).toBeInTheDocument()

    fireEvent.change(rulesetSelect, { target: { value: "crazykrieg" } })
    expect(screen.queryByRole("option", { name: "1229 - LLM GPT-4.5 Nano (bot)" })).not.toBeInTheDocument()
    expect(screen.getByText("No bots support this ruleset.")).toBeInTheDocument()
  })
